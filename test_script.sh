...
echo -e "\n== Loading XDP =="
./bin/xdp-firewall -p "$CORRECT_PASSWORD" "$INTERFACE" "$TEST_PORT" -S
if [ $? -ne 0 ]; then
...
./bin/packet_sender -p "$CORRECT_PASSWORD" "$TEST_IP" "$TEST_PORT" "$MESSAGE"

sleep 1

echo -e "\n== Sending packet with wrong password. Expect no tcpdump info after it =="
./bin/packet_sender -p "$WRONG_PASSWORD" "$TEST_IP" "$TEST_PORT" "$MESSAGE"

sleep 1

echo -e "\n== Sending packet with wrong port and right password. Expect no tcpdump info after it =="
./bin/packet_sender -p "$CORRECT_PASSWORD" "$TEST_IP" "$WRONG_PORT" "$MESSAGE"
